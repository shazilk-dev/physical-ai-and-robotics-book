...
```mermaid
graph LR
    A[📝 Node Structure] --> B[📤 Publisher Pattern]
    B --> C[📥 Subscriber Pattern]
    C --> D[🔧 Service Pattern]
    D --> E[🎯 Action Pattern]
    E --> F[🚀 Lab 1: Build Your First Node]
...
- ✅ Create ROS 2 nodes from scratch
- ✅ Publish sensor data at specific rates (e.g., 30Hz camera, 100Hz IMU)
- ✅ Subscribe to topics and process incoming data
- ✅ Call services for one-time actions (enable motors, save map)
- ✅ Use actions for long-running tasks (navigate, grasp)
...
- ✅ **Python basics**: Classes, functions, imports, `self`
- ✅ **ROS 2 installed**: Completed Module 1 Overview setup guide
- ✅ **Core concepts**: Read Section 1.1.1 (nodes, topics, services, actions)
...
- 📷 Camera node publishes images at 30 Hz
- 🧭 IMU node publishes orientation at 200 Hz
...
import LabCallout from '@site/src/components/LabCallout/LabCallout';

<LabCallout
labId="lab01-ros2-basics"
title="Lab 1: Your First ROS 2 Node - Heartbeat Publisher"
difficulty="Beginner"
duration="30 minutes"
prerequisites={[
"Section 1.1.1: ROS 2 Architecture & Developer Workflow",
"Section 1.1.2: rclpy Patterns (this section)",
"Basic Python knowledge (classes, functions)"
]}
description="Apply the publisher pattern you just learned! Build a complete ROS 2 package with a heartbeat node, implement timer callbacks, and verify message publishing with CLI tools. This lab includes starter code with TODOs, automated tests, and step-by-step instructions."
...
class HeartbeatListener(Node):
    ...
    def heartbeat_callback(self, msg):
        ...
        if 'READY' in msg.data:
            self.get_logger().info('✅ Robot is operational')
...
class EmergencyStopClient(Node):
    ...
    def call_emergency_stop(self):
        ...
        if response.success:
            ...
            self.get_logger().error(f'❌ Emergency stop failed')
```
...
**Lifecycle nodes** provide explicit state management (configure → activate → deactivate → cleanup).
...
self.create_publisher(String, 'status', 10)  # → /node_namespace/status
```
...
✅ **Publishers** send data without knowing receivers (topics)  
✅ **Subscribers** receive data automatically via callbacks  
✅ **Services** provide request/response for stateful operations  
✅ **Actions** handle long tasks with feedback and cancellation  
✅ **Timers** trigger periodic callbacks (e.g., control loops)  
✅ **Always store subscription references** to prevent garbage collection

## 🧪 Lab Task: Build Your First Node
