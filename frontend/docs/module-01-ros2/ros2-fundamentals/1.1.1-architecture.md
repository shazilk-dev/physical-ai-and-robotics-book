...
graph LR
    A[👨‍🍳 Chef<br/>Brain]
    B[🍽️ Waiter<br/>Sensors]
    C[🔥 Cook<br/>Motors]
    D[📋 Order Tickets<br/>ROS 2 Messages]
...
- **Waiter** (Sensors) takes orders from customers → sends to kitchen
- **Chef** (Brain) reads orders → decides what to cook
- **Cook** (Motors) receives instructions → prepares food
...
**Without ROS 2:** You'd need to write custom networking code for every connection (HTTP? TCP? UDP? Shared memory?). Nightmare! 😱

**With ROS 2:** Each program publishes/subscribes to named "channels" (called **topics**). ROS 2 handles all the networking automatically. ✅
...
1. **Camera Driver** captures 640×480 RGB image
...
```mermaid
graph LR
    A[📷 Camera Node<br/>Captures images]
    B[🧠 Detector Node<br/>Finds objects]
    C[🗺️ Planner Node<br/>Plans path]
    D[🦾 Motor Node<br/>Moves robot]
```
...
- ✅ **Modularity**: Replace camera without touching detector code
- ✅ **Debugging**: If detector crashes, camera keeps running
- ✅ **Scalability**: Run each node on different computers if needed
...
**When to use Topics?**  
✅ Continuous data streams (sensor readings, commands)  
✅ Data that many nodes need (camera feed)  
✅ No response needed (just broadcast and move on)
...
✅ One-time actions (enable/disable, save, load)  
✅ You need confirmation (success/failure)  
✅ Stateful operations (changing configuration)
...
**When to use Actions?**  
✅ Tasks that take seconds/minutes (navigation, grasping)  
✅ You want progress updates (how much longer?)  
✅ User might cancel (stop navigating midway)
...
import QuizAccordion from '@site/src/components/Quiz';
...
options: [
"python setup.py install → source devel/setup.bash → rosrun",
"colcon build → source install/setup.bash → ros2 run",
"catkin_make → source devel/setup.bash → ros2 launch",
"cmake → make install → ros2 run"
],
correctAnswer: 1,
...
✅ **ROS 2 is middleware**, not an OS - it handles communication between processes  
✅ **DDS provides decentralized discovery** - no single point of failure  
✅ **Topics for streams** (sensors, odometry), **services for state** (save map), **actions for tasks** (navigate)  
✅ **QoS policies matter** - sensor data tolerates loss, commands demand reliability  
✅ **colcon build, source, run** - the three-step development loop
