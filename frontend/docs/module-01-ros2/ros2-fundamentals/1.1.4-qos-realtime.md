...
```mermaid
graph LR
    A[⏱️ Real-Time<br/>Basics] --> B[📊 Latency<br/>Budgeting]
    B --> C[⚙️ QoS<br/>Profiles]
    C --> D[🎯 Optimized<br/>Robot System]
...
- ✅ Understand soft vs. hard real-time
- ✅ Budget latency for control loops
- ✅ Choose correct QoS settings for different data types
- ✅ Optimize communication for low-latency performance
...
Every component in the perception → planning → actuation pipeline adds latency. Budget carefully for your target control frequency.

### Example: Humanoid Walking Control Loop (1 kHz / 1 ms period)

```mermaid
graph LR
    A[IMU Data] -->|100 μs| B[State Estimator]
    B -->|200 μs| C[Balance Controller]
    C -->|300 μs| D[Inverse Kinematics]
    D -->|100 μs| E[Motor Commands]
    E -->|300 μs| F[Actuators]
...
- 🚀 **BEST_EFFORT** = Regular mail (fast but can lose packets)
...
**Problem:** Subscriber uses RELIABLE, publisher uses BEST_EFFORT → **No connection**
...
**Next:** [Chapter 1.2: URDF, Xacro & Robot Description](../02-urdf-xacro/1.2.1-urdf-basics.md) – Learn how to define your robot's physical structure and kinematics.

**Module 1 Progress:** 🎉 You've completed Chapter 1.1! You can now:
...
**Ready for Lab 1?** Head to [Lab 1: ROS 2 Basics](/labs/lab01-ros2-basics/README) to build your first node!
