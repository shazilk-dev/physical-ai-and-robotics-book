...
graph TB
    ...
    subgraph Processing["Depth Processor"]
        ...
        F[Convert to depth<br/>Z = focal_length × baseline / disparity]
    end
        ...
        try:
            ...
            if len(valid_depths) > 0:
                ...
                if min_distance < 0.5:  # Closer than 50 cm
                    self.get_logger().warn(f'⚠️  OBSTACLE! Closest object: {min_distance:.2f}m')
...
class ObstacleDetector(Node):
    ...
    def depth_callback(self, msg):
        ...
        if dist_center < self.DANGER_ZONE:
            ...
            warning_msg.data = f'⚠️  Caution: Object at {dist_center:.2f}m ahead'
            self.get_logger().warn(warning_msg.data)
        elif dist_left < self.WARNING_ZONE:
            warning_msg.data = f'⚠️  Obstacle on LEFT: {dist_left:.2f}m'
            self.get_logger().warn(warning_msg.data)
        elif dist_right < self.WARNING_ZONE:
            warning_msg.data = f'⚠️  Obstacle on RIGHT: {dist_right:.2f}m'
            self.get_logger().warn(warning_msg.data)
        else:
