...
**Answer:** **Harmonic drive gearboxes** multiply motor torque by 50-160×, enabling small motors to produce massive output torque.
...
- Motor mass: 100 Nm / 0.15 = 667× 100g = 66.7 kg ❌
...
**Basic Relationship:**
```
Torque_out = Torque_in × Gear Ratio × Efficiency
Speed_out = Speed_in / Gear Ratio

Example:
Motor: 2 Nm @ 3000 RPM
Gearbox: 50:1, 85% efficient
Output: 2 × 50 × 0.85 = 85 Nm @ 60 RPM
```
...
```
Stage 1: 5:1 ─→ Stage 2: 5:1 ─→ Stage 3: 4:1 ─→ Output
         │              │              │
...
- Harmonic drive: $2,400 (13× more expensive!)
```
...
**Selection:**
- Need 150 Nm output → CSF-17-50 is marginal (100 Nm rated)
- Choose CSF-20-50 for 1.5× safety factor ✅
...
12.6 rad/s × (180/π) = 722°/s ✅

Requirement: 360°/s (full rotation in 1 second)
Margin: 722 / 360 = 2× (comfortable!)
```
...
Ratio: 0.02 / 0.015 = 1.3:1 ✅ (acceptable: 0.5 to 5)
```

**Why it matters:** Mismatched inertia → poor control performance (oscillations, overshoots).
...
- ✅ Budget < $500 per joint
- ✅ Gear ratio < 20:1
- ✅ Backdrivability is desired (for safety)
- ✅ Efficiency > 85% is critical (battery life)
...
**Multi-Stage:**
```
Stage 1: 4:1 → Stage 2: 5:1 → Total: 4 × 5 = 20:1
```
...
**Cascading Stages:**
```
10:1 (Stage 1) × 5:1 (Stage 2) = 50:1 total
Efficiency: 0.92 × 0.92 = 85%
Weight: 420g + 420g = 840g (similar to harmonic!)
Cost: $220 + $220 = $440 (5× cheaper than harmonic!)
```
...
**Formula:**
```
P_loss = P_in × (1 - Efficiency)

Example:
Input power: 500W
Efficiency: 80%
Loss: 500 × (1 - 0.80) = 100W of heat!
```

**Impact:** 100W heats up gearbox → thermal expansion → binding → failure.
...
**Prevention:** Size gearbox with 1.5-2× safety factor, monitor temperature.
...
if not options:
    print("❌ No suitable gearboxes found within budget!")
else:
    print(f"✅ Found {len(options)} suitable options:\n")
    ...
    print(f"💰 Best value: {best_cost['model']} (${best_cost['cost']})")
    print(f"⚖️  Lightest: {best_weight['model']} ({best_weight['weight']:.2f} kg)")
    print(f"⚡ Most efficient: {best_eff['model']} ({best_eff['efficiency']*100:.0f}%)")
```
...
=== Recommendations ===
💰 Best value: PLE-64-10 + PLE-64-5 ($460)
⚖️  Lightest: CSF-17-50 (0.58 kg)
⚡ Most efficient: Planetary + Harmonic tie (85%)
```
...
1. ✅ **Harmonic drives** offer zero backlash and high torque density (best for precision joints)
2. ✅ **Planetary gearboxes** are cheaper and more efficient (best for budget builds)
3. ✅ **Gear ratio** multiplies torque but reduces speed (choose based on joint requirements)
4. ✅ **Efficiency losses** generate heat (plan thermal management for high-power joints)
5. ✅ **Backdrivability** is important for safety (prefer planetary or series elastic actuators)

---

### Gearbox Selection Checklist

- [ ] Calculate required output torque (motor torque × gear ratio × efficiency)
- [ ] Verify gearbox rated torque > required torque (with 1.5× safety factor)
...
**Option B:** Planetary cascade (10:1 × 5:1, 85% eff, $400, 0.8 kg)
...
Continue to **[Section 2.2.3: Series Elastic Actuators](./2.2.3-series-elastic-actuators.md)** to discover how humanoids safely interact with humans and unknown environments! 🤝🦾
