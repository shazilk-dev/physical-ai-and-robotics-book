...
**Consequence:** When a humanoid bumps into a wall, it doesn't know to stop → damage or injury!
...
- ✅ Compliant behavior (spring absorbs impacts)
- ✅ Safe human interaction (gentle touch, not rigid collision)
...
Problem: If obstacle is at 40°, motor fights it → high forces!
```
...
Benefit: If obstacle appears, force stays at 10 N → safe!
```
...
```
Motor → Gearbox → Spring → Output (Robot Link)
                    ↓
...
**Hooke's Law:**
```
Force = Spring Stiffness × Deflection

F = k × Δx

Where:
- k = Spring constant (N/m or Nm/rad)
- Δx = Spring compression (meters or radians)
```

**In Practice:**
```
Encoder 1 (motor side): θ_motor = 250°
Encoder 2 (output side): θ_output = 245°
Deflection: Δθ = 250° - 245° = 5° = 0.087 rad

Spring stiffness: k = 200 Nm/rad
Torque: τ = 200 × 0.087 = 17.4 Nm
```
...
k = 100 Nm / (5° × π/180) = 100 / 0.087 = 1150 Nm/rad

Verify resolution:
Encoder: 0.05° = 0.00087 rad
Force resolution = k × 0.00087 = 1150 × 0.00087 = 1.0 Nm ✅ (meets 0.5 Nm requirement)
```
...
**Equation:**
```
τ = k_p × (θ_desired - θ_actual) + k_d × (ω_desired - ω_actual)
...
```
Jump height: 1.2m
Landing force: ~3× body weight (reduced from 6× without SEAs)
...
4. Encoders: 2× AS5048 magnetic encoders ($24)
...
```
[ Motor ] ─→ [ Gearbox ] ─→ [ Encoder 1 ] ─→ [ Spring ] ─→ [ Encoder 2 ] ─→ [ Output Shaft ]
                                                    ↑
                                             Compliance here!
```
...
Result: k = 30 Nm/rad ✅ (close enough!)
...
- **Example:** Drone propeller control ❌
...
- **Example:** Micro-surgery robots ❌

**3. Very Low-Torque Joints**
- Spring must be very soft → bulky spring for small deflection
- Not practical for grippers or small fingers
- **Example:** Miniature robot hands ❌
...
```
Measure motor current → Estimate torque (τ = K_t × I)
...
1. ✅ **SEAs enable force control** by measuring spring deflection
2. ✅ **Compliance improves safety** (absorbs impacts, gentle interaction)
3. ✅ **Spring stiffness is a trade-off** (high k = good position, low k = good force sensing)
4. ✅ **Dual encoders are required** (one on each side of spring)
5. ✅ **SEAs reduce energy consumption** (spring stores/releases energy during locomotion)
...
You now understand:
- ✅ BLDC motors and encoders
- ✅ Harmonic drives and gearboxes
- ✅ Series elastic actuators for force control

**Next:** **Chapter 2.3: Edge Compute Architecture** - Learn how to deploy AI models on NVIDIA Jetson for real-time perception!

Continue to **[Chapter 2.3: Edge Compute](../edge-compute/2.3.1-jetson-architecture.md)** 🖥️⚡
