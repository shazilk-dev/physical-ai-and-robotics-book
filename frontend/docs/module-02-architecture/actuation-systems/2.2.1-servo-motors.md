...
**Advantages:**
- ✅ **High efficiency** (85-90% → less battery drain, less heat)
- ✅ **High power density** (3-5 kW/kg for good motors)
- ✅ **No brushes** → longer lifespan (>10,000 hours)
- ✅ **Precise control** with encoders (position accuracy < 0.1°)
- ✅ **Scalable** (same technology works from 10W to 10kW)

**Disadvantages:**
- ❌ **Requires ESC (Electronic Speed Controller)** → adds cost + complexity
- ❌ **Needs encoder** for position control (extra sensor)
- ❌ **More expensive** than brushed ($100-500 vs $20-50)
...
**Operating Principle:**
```
Step 1: Energize coil A → Rotor turns 60°
Step 2: Energize coil B → Rotor turns another 60°
Step 3: Energize coil C → Rotor turns another 60°
...
Motor: 1000 KV
Voltage: 24V
Max RPM = 1000 × 24 = 24,000 RPM
```

**Rule of Thumb:**
- High KV (>1500) → Fast, low torque (drones, fans)
- Low KV (&lt;500) → Slow, high torque (robot joints)
...
```
Torque (Nm) = K_t × Current (A)

Example:
K_t = 0.05 Nm/A
Current = 20A
Torque = 0.05 × 20 = 1.0 Nm
```

**Important:** K_t and KV are inversely related:
```
K_t ≈ 8.3 / KV  (approximate formula)
```
...
Peak Torque: 6.5 Nm (3.25× higher!)
```
...
Torque from forearm:
τ_forearm = (0.5 kg × 9.81) × (0.25/2) m = 0.61 Nm

Torque from payload:
τ_payload = (1.0 kg × 9.81) × 0.25 m = 2.45 Nm

Total torque (static):
τ_total = 0.61 + 2.45 = 3.06 Nm
```
...
I = m × r² = 1.5 kg × (0.125 m)² = 0.023 kg·m²

Desired acceleration: 180°/s² = 3.14 rad/s²
τ_dynamic = 0.023 × 3.14 = 0.07 Nm (small!)
```
...
```
τ_required = (τ_static + τ_dynamic) × Safety Factor
τ_required = (3.06 + 0.07) × 2.0 = 6.26 Nm
```

**Safety factor rationale:**
- 2.0× for smooth operation (not running at max power)
...
```
Send motor "20% power" → Hope it moves to correct position → Pray! 🤞
```
**Problem:** Motor position depends on load, friction, battery voltage → highly inaccurate!

**Closed-Loop Control (With Encoder):**
```
1. Read encoder: Current position = 45°
2. Target position = 90°
3. Error = 90° - 45° = 45°
4. Apply torque proportional to error (PID control)
5. Repeat at 1 kHz → Precise positioning! ✅
```
...
| **Absolute (Magnetic)** | 12-14 bits (0.09°) | $30-100 | SPI, I2C | Robot joints (no homing needed) |
| **Absolute (Optical)** | 16-20 bits (0.001°) | $200-1000 | Serial | CNC machines, precision robotics |
...
Example:
Encoder: 2048 CPR
Resolution = 360° / 2048 = 0.176° ≈ 0.18°
```

**With Quadrature Decoding:** 4× resolution (count rising + falling edges on both channels)
```
Effective CPR = 2048 × 4 = 8192
Resolution = 360° / 8192 = 0.044° ✅
```

**Typical Requirements:**
- Gripper: 0.5° resolution (500 CPR encoder)
- Arm joints: 0.1° resolution (2000 CPR encoder)
- Precision joints: 0.01° resolution (20,000 CPR encoder or absolute)
...
Interface: SPI (10 MHz clock → 10 kHz update rate)
Cost: $12 per encoder
Power: 3.3V @ 10 mA
```
...
**Requirements:**
- Support full robot weight: 35 kg × 9.81 = 343 N
- Knee-to-CoM distance: ~0.3m (worst case: standing on one leg)
- Required torque: 343 N × 0.3 m = 103 Nm
- Desired speed: 180°/s (3.14 rad/s)
...
Output torque: 30 Nm × 4.3 = 129 Nm ✅
...
- Required torque: 5 kg × 9.81 × 0.15 m = 7.4 Nm
- High-speed motion: 360°/s (6.28 rad/s)
...
Output torque: 2.0 Nm × 5 × 0.85 (efficiency) = 8.5 Nm ✅
Output speed: (24,000 RPM / 1000 KV) / 5 = 4,800 RPM = 502 rad/s
Cost: $180 (motor) + $85 (gearbox) = $265
```

**Design Trade-off:** High-speed motor with low gear ratio → fast wrist motion, adequate torque.
...
control_config:
  ...
  max_erpm: 48000  # Max electrical RPM = KV × V × pole_pairs
```

**Tuning:** Run VESC detection wizard → Auto-measures R, L, flux linkage.
...
**Power Loss:**
```
P_loss = I² × R + (Torque × Speed) × (1 - Efficiency)

Example:
Current: 30A
Resistance: 0.05 Ω
Motor losses: 30² × 0.05 = 45W of heat!
```
...
**Failure Mode:** Overheating → Demagnetization of permanent magnets → Motor death 💀
...
class MotorController(Node):
    ...
    def target_callback(self, msg):
        ...
        self.get_logger().info(f'New target: {self.target_position:.2f}°')
...
1. ✅ **BLDC motors** are ideal for humanoid robots (high efficiency, precise control)
2. ✅ **Motor torque = K_t × Current** - Select motors based on required torque
3. ✅ **Gear reduction** multiplies torque but reduces speed
4. ✅ **Absolute encoders** (AS5048) eliminate homing, improve reliability
5. ✅ **PID control at 1 kHz** is standard for precise position control
...
- [ ] Select encoder with resolution < 0.1° for precision joints
...
3. Total torque with 2× safety factor
...
**Question:** You need 0.5° position accuracy at the end-effector.
...
Continue to **[Section 2.2.2: Harmonic Drives & Gear Reduction](./2.2.2-harmonic-drives.md)** to discover why humanoids use special "harmonic drive" gearboxes! ⚙️🦾
