...
Problem: Jetson needs stable 5V (±5%) → 4.75V to 5.25V
If you connect 36V battery directly → BOOM! 💥 (destroyed Jetson)
```
...
**For Robotics:** **Switching regulators** (much more efficient → less heat, longer battery life).
...
```
Input (36V) ──┬──[Switch]──┬──[Inductor]── Output (5V)
              │            │
             GND         [Cap]
                          │
                         GND
...
**Circuit:**
```
36V Battery ──┬──[100uF Cap]──[TPS54620]──[10uF Cap]── 5V Output (to Jetson)
              │                   │
             GND                 GND

Output current: 3A nominal, 6A available (2× headroom for peaks)
...
**Example:**
```
USB power bank (3.7V Li-ion cell) → Boost to 5V for Raspberry Pi
Robot battery (12V) → Boost to 24V for high-torque motor driver
```
...
```
Input (12V) ──[Inductor]──┬──[Diode]── Output (24V)
                          │
                      [Switch]
                          │
                         GND
...
12V × I_in = 24V × I_out
I_out = (12 / 24) × I_in = 0.5 × I_in

If input current is 10A → output current is only 5A!
```
...
**Recommended Module:** **Vicor BCM** (12V→24V, 150W, 95% efficiency, $85)
...
6S Li-ion battery:
- Fully charged: 25.2V
- Nominal: 22.2V  ← Target: 24V for motor
- Depleted: 18.0V

At full charge: Need buck (25.2V → 24V)
At depletion: Need boost (18.0V → 24V)
```
...
**Centralized (Simpler):**
```
Battery ─→ Power Distribution Board ─┬─→ 5V Rail (Jetson, sensors)
                                     ├─→ 12V Rail (fans, cameras)
                                     └─→ 36V Rail (motor drivers)
...
```
Battery ─┬─→ Local Buck (Jetson)
         ├─→ Local Buck (Cameras)
         └─→ Motor Drivers (direct battery connection)
...
Battery (36V)
     │
     ├─────[100A Fuse]────[Main Switch]────┬───────────────────┐
     │                                     │                   │
     │                              [Buck 36V→12V, 10A]  [Buck 36V→5V, 5A]
     │                                     │                   │
     │                                  12V Rail             5V Rail
     │                                     │                   │
     │                              ┌──────┴─────┐      ┌─────┴──────┐
     │                              │            │      │            │
     │                            Fans      Cameras   Jetson     Sensors
     │
...
|-----------|---------|-------------|-----------|-----------|
| **Jetson Orin** | 5V | 3A | 15W | Buck 36V→5V (5A) |
| **RealSense (4×)** | 12V | 1.5A | 18W | Buck 36V→12V (10A) |
| **Fans (2×)** | 12V | 0.5A | 6W | (same 12V rail) |
| **IMU** | 3.3V | 0.05A | 0.2W | LDO 5V→3.3V |
| **Motor ESCs (12×)** | 36V | 50A peak | 1800W | Direct battery |

**Converter Sizing:**
```
5V Rail:
- Load: 3A (Jetson) + 0.05A (IMU via LDO) = 3.05A
- Converter: 5A (65% headroom) ✅

12V Rail:
- Load: 1.5A (cameras) + 0.5A (fans) = 2A
- Converter: 10A (5× headroom for camera inrush current) ✅
```
...
For 18 AWG wire (1.02 mm² cross-section):
R = 1.68×10⁻⁸ × 1m / (1.02×10⁻⁶) = 0.0165 Ω/m
```
...
**Voltage Drop:**
```
Wire resistance: 0.0165 Ω/m × 1m = 0.0165 Ω

Voltage drop (one-way):
V_drop = I × R = 3A × 0.0165 Ω = 0.05 V

Total drop (round-trip: positive + return):
V_drop_total = 2 × 0.05V = 0.10V

Voltage at load:
V_load = 5.0V - 0.10V = 4.90V ✅ (within ±5% spec)
```
...
| AWG | Diameter (mm) | Resistance (Ω/m) | Max Current (Chassis) | Use Case |
|-----|---------------|------------------|----------------------|----------|
...
```
Battery GND ──┬─→ Jetson GND
              ├─→ Camera GND
              └─→ Motor Driver GND (via separate wire!)
...
**Multi-Point Ground:**
```
Battery GND ──┬─→ Jetson GND ──┬─→ Camera GND
              │                └─→ Sensor GND
              └─→ Motor Driver GND
...
```
Battery GND ──┬─→ [Digital Ground Plane] ─→ Jetson, sensors
              └─→ [Motor Ground Plane] ─→ ESCs, motors (isolated!)
```
...
```
Motor ESC switches at 20 kHz → 20 kHz ripple on 36V rail
This couples into 5V rail via parasitic capacitance → Jetson sees noise!
```

**Solution: LC Filter (Low-Pass)**

```
36V from Battery ──[Inductor 10uH]──┬──[Cap 100uF]── 36V to Motors
                                    │
                                   GND

Cutoff frequency:
f_c = 1 / (2π√(LC))
f_c = 1 / (2π√(10×10⁻⁶ × 100×10⁻⁶)) = 1591 Hz

Blocks 20 kHz noise ✅, passes DC and low-frequency motor currents ✅
```
...
**Placement:**
```
5V Rail ──[Ferrite Bead]── Jetson Orin

Impedance:
- DC (0 Hz): 0 Ω (no voltage drop)
- 100 kHz: 10 Ω
- 10 MHz: 600 Ω (blocks EMI from switching supplies)
```
...
I_inrush = V / R_wire
I_inrush = 36V / 0.01Ω (wire resistance) = 3600A! ❌
...
```
Battery ──[NTC Thermistor]── Power Distribution Board

Cold resistance: 10 Ω (limits inrush to 36V / 10Ω = 3.6A ✅)
Hot resistance: 0.1 Ω (minimal loss during operation)
```

**How it works:**
1. Initially cold → high resistance → limits inrush
2. Current flows → heats thermistor → resistance drops
3. After 1-2 seconds → thermistor is hot → nearly zero resistance

**Recommended Part:** Ametherm SL32 (10Ω, 32A surge capacity, $3)
...
```
E-Stop Button ─→ Relay (NO contact) ─→ Main Power

When pressed: Relay opens → Battery disconnected from all loads
```
...
```
Battery (+) ──┬──[Main Fuse 100A]──[E-Stop Relay]──┬── Power Distribution
             │                                      │
            GND ──────────────────────────────────── GND

E-Stop Button ──[12V Coil]── Relay (when button pressed → coil de-energized → relay opens)
```
...
2. Fault detection (overcurrent → short circuit)
...
```
36V Battery ──[ACS712]── Power Distribution
                │
...
Output voltage:
V_out = 2.5V + (I × Sensitivity)

For ACS712-50A:
Sensitivity = 40 mV/A
At 30A: V_out = 2.5V + (30A × 0.040V/A) = 3.7V
```
...
1. ✅ **Buck converters** step down voltage (36V→5V) with 85-95% efficiency
2. ✅ **Boost converters** step up voltage but reduce output current
3. ✅ **Wire gauge** must be sized to limit voltage drop < 3%
4. ✅ **Star grounding** isolates noisy motor power from sensitive electronics
5. ✅ **LC filters** block motor switching noise from power rails
6. ✅ **E-stop relay** must disconnect all power instantly for safety
...
**Scenario:**
- 5V rail, 18 AWG wire (0.0165 Ω/m)
...
2. Voltage at load
3. Is this acceptable? (5V ±5% = 4.75-5.25V)
...
You now understand:
- ✅ Battery systems and BMS design
...
<!-- Continue to **[Lab 4: Motor Control](../../labs/lab04-motor-control.md)** 🦾⚡ -->
<!-- Lab 4 coming soon -->
