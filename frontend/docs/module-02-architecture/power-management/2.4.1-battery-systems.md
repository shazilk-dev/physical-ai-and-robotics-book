...
|-----------|----------------|---------------|------------|------|--------|
| **LiPo (Li-Polymer)** | 150-200 Wh/kg | High (100C) | 300-500 | $$ | ⚠️ Moderate |
| **NMC (Li-ion)** | 200-260 Wh/kg | Medium (30C) | 1000-2000 | $$$ | ✅ Good |
| **LiFePO4 (LFP)** | 90-120 Wh/kg | Low (10C) | 2000-5000 | $$ | ✅ Excellent |
| **Lead-Acid** | 30-50 Wh/kg | Very Low | 200-300 | $ | ✅ Good |

**For Humanoid Robots:** **NMC (Li-ion)** is the industry standard (Tesla Optimus, Unitree G1, Figure 02).

**Why NMC?**
- ✅ Best energy density (longer runtime)
- ✅ Long cycle life (>1000 charges)
- ✅ Acceptable safety (with proper BMS)
- ❌ More expensive than LiPo (but worth it for production robots)
...
```
Pros:
- High discharge rate (100C = 100× capacity in amps)
- Cheap ($30-80 for 5000mAh 6S)
- Readily available (drone/RC market)

Cons:
- Short lifespan (300-500 cycles → 1 year)
...
- Long lifespan (2000 cycles → 5+ years)
...
- Motors (12× joints @ 50W avg): 600W
- Jetson Orin (perception): 30W
- RealSense cameras (4×): 20W
- Cooling fans: 10W
- Microcontroller (STM32): 5W

Total Average Power: 665W
Peak Power (jumping, running): 1500W (2.25× average)
```
...
Energy Required:
E = Power × Time
E = 665W × 2 hours = 1330 Wh

With 20% safety margin (degradation, cold weather):
E_required = 1330 Wh × 1.2 = 1596 Wh
```
...
Energy per cell: 5 Ah × 3.7V = 18.5 Wh

Cells needed: 1596 Wh / 18.5 Wh = 86 cells

Configuration: 6S14P (6 series × 14 parallel)
- Voltage: 6 × 3.7V = 22.2V nominal, 25.2V max
- Capacity: 14 × 5Ah = 70 Ah
- Energy: 22.2V × 70 Ah = 1554 Wh ✅
- Mass: 86 cells × 90g/cell = 7.7 kg
- Cost: 86 cells × $2.50/cell = $215

Problem: 86 cells require complex BMS! ❌
```
...
Energy per cell: 50 Ah × 3.6V = 180 Wh
Mass per cell: 850g

Cells needed: 1596 Wh / 180 Wh = 9 cells (round up to 10 for margin)

Configuration: 10S1P (10 series)
- Voltage: 10 × 3.6V = 36V nominal, 42V max
- Capacity: 50 Ah
- Energy: 36V × 50 Ah = 1800 Wh ✅
- Mass: 10 cells × 850g = 8.5 kg
- Cost: 10 cells × $40/cell = $400

Simpler BMS (only 10 cells to manage) ✅
Higher voltage (more efficient for motor drivers) ✅
```
...
NMC cells typically support 1C-3C continuous discharge ✅
Peak bursts up to 5C for &lt;10 seconds ✅
...
5. **Temperature Monitoring** - Shuts down if cells overheat (>60°C)
...
| Spec | Value | Notes |
|------|-------|-------|
| **Output Voltage** | 42V | 10S × 4.2V/cell |
...
class BMSMonitor(Node):
    ...
    def read_bms(self):
        ...
        if msg_response:
            ...
            temperature = msg_response.data[5] - 40  # °C (offset by 40)
            ...
            self.get_logger().info(f'Battery: {voltage:.1f}V, {current:.1f}A, {soc}%, {temperature}°C')
...
```
Overcharge → Cell overheats → Internal short → More heat → Explosion! 💥
```

**Prevention Strategies:**

1. **BMS with Temperature Monitoring**
   - Shut down charging if cell temp > 45°C
   - Shut down discharging if cell temp > 60°C
...
2. Charger failed to stop at 4.2V/cell → overcharged to 4.4V
3. Cell vented, ignited other cells
4. Fire destroyed drone + workbench
```

**Lessons Learned:**
- ✅ Always use BMS with overcharge protection
- ✅ Never charge batteries unattended
- ✅ Store batteries in fireproof bag (LiPo safety bag, $15)
- ✅ Keep fire extinguisher nearby (Class D for lithium fires)
...
CoM impact: Lowers robot CoM by 8 cm ✅
```
...
Cycle 500:  90% capacity (45 Ah) ← 10% loss
Cycle 1000: 80% capacity (40 Ah) ← 20% loss (end of life)
Cycle 2000: 70% capacity (35 Ah)
```

**Factors Accelerating Degradation:**
- ❌ High charge/discharge rates (>1C)
- ❌ High temperature (>45°C during operation)
- ❌ Full charge storage (keeping battery at 100% for weeks)
- ❌ Deep discharge (running battery to 0%)
...
Benefit: 2× longer lifespan (1000 → 2000 cycles)
Cost: 40% less usable energy

For 1800 Wh pack:
Usable energy = 1800 × 0.6 = 1080 Wh (still acceptable for 1.5 hour runtime)
```
...
Storage temp: 15°C to 25°C (optimal)

If operating in hot environment (>35°C):
...
1. ✅ **NMC chemistry** is best for production humanoids (high energy density, long lifespan)
2. ✅ **BMS is mandatory** for safety (overcharge, over-discharge, thermal protection)
3. ✅ **Battery sizing:** Power budget × Runtime × 1.2 safety margin
4. ✅ **Active balancing** BMS is preferred for 10S+ configurations
5. ✅ **Low battery placement** improves robot stability (pelvis, not chest!)
...
Continue to **[Section 2.4.2: Thermal Design](./2.4.2-thermal-design.md)** 🔥❄️
