...
**Key Insight:** Different tasks have different requirements → hybrid architecture (MCU for control, Jetson for perception).
...
**Rule of Thumb:**
- **10 TOPS** ≈ 30 FPS for YOLOv8 object detection (640×640)
- **40 TOPS** ≈ 10 FPS for RT-2 VLA (vision-language-action model)
- **200 TOPS** ≈ Real-time semantic segmentation + VLA + SLAM simultaneously
...
- Limitation: No Tensor cores → 5× slower for Transformers
...
│                                                           │
│  ┌──────────────┐         ┌────────────────────────┐   │
│  │ CPU Complex  │         │   GPU (CUDA + Tensor)   │   │
│  │ 12-core ARM  │         │   2048 CUDA cores       │   │
│  │ Cortex-A78   │         │   64 Tensor cores       │   │
│  │ @2.2 GHz     │         │   @1.3 GHz              │   │
│  └──────────────┘         └────────────────────────┘   │
│         │                            │                   │
│         └────────┬───────────────────┘                   │
...
│ ┌──▼───┐   ┌────▼────┐   ┌───▼────┐                    │
│ │Video │   │ Deep    │   │ Vision │                    │
│ │Encode│   │ Learning│   │Accel.  │                    │
│ │/Decode│  │Acceler. │   │        │                    │
│ └──────┘   └─────────┘   └────────┘                    │
│                                                           │
│  I/O: PCIe Gen 4, USB 3.2, Ethernet 10GbE, CAN, GPIO   │
└───────────────────────────────────────────────────────────┘
```
...
```
1 CUDA core ≈ 1 floating-point operation per clock cycle
2048 cores × 1.3 GHz = 2.6 TFLOPS (FP32)
```

**Tensor Cores:** Specialized for matrix multiplication (AI workloads)
```
1 Tensor core ≈ 256 operations per clock cycle (INT8)
64 Tensor cores × 1.3 GHz × 256 = 21 TOPS (FP16) or 42 TOPS (INT8)
```
...
- Jetson GPU: 2,600 GFLOPS (87× faster for parallel tasks!)
```
...
```
CPU ───┬──→ Shared Memory (64 GB LPDDR5)
GPU ───┘

Bandwidth: 204 GB/s (13× faster!)
```
...
- Camera 1: Object detection (YOLOv8) → Run on DLA
- Camera 2: Semantic segmentation (DeepLabV3) → Run on GPU
...
**Key Insight:** Keeping data in L1/L2 cache is **50× faster** than DRAM access!
...
Input image: 1.2 MB (1920×1080 RGB)
...
- Frequent CPU↔GPU transfers (inefficient code)
...
```
If workload is light → Jetson automatically reduces clock speed → Saves power
If workload is heavy → Jetson boosts to max → Delivers performance
```
...
**Temperature Limits:**
```
Safe operating: 0-70°C
Thermal throttling: 70-85°C (performance reduced)
...
GPU (2048 CUDA cores): 0.5 ms (20× faster!)
```
...
**Hardware:**
- Jetson AGX Orin 32GB ($1,999)
- 4× Intel RealSense D435i cameras
- 1× IMU (BMI088)

**Software Stack:**
```
Camera drivers → ROS 2 Humble
YOLOv8 (object detection) → TensorRT (30 FPS)
ORB-SLAM3 (visual SLAM) → CUDA-accelerated (60 Hz)
OpenVLA (vision-language-action) → TensorRT FP16 (5 FPS)
```
...
```
VLA Model (RT-2): 200 TOPS → Jetson GPU
Motor Control: 10W MCU (STM32H7)
Sensor Fusion: 50 TOPS → Jetson DLA
```
...
1. ✅ **Jetson enables real-time AI** on robots (no cloud latency!)
2. ✅ **Tensor cores accelerate deep learning** (40-275 TOPS depending on model)
3. ✅ **Unified memory** eliminates CPU↔GPU copy overhead
4. ✅ **Power modes** trade performance for battery life (15-60W range)
5. ✅ **JetPack SDK** includes CUDA, TensorRT, OpenCV out-of-the-box
...
- [ ] Check power budget (battery capacity × runtime target)
...
**Scenario:**
- You're processing 4K video (3840×2160 RGB) at 30 FPS
- Each frame = 3840 × 2160 × 3 bytes = 24.9 MB
...
Continue to **[Section 2.3.2: CUDA Optimization](./2.3.2-cuda-optimization.md)** to learn memory coalescing, kernel fusion, and profiling! 🚀⚡
