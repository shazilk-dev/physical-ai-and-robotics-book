...
**Result:** 3-5× speedup with **zero accuracy loss** (for FP16) or &lt;2% loss (for INT8)!
...
TensorRT Builder (optimization)
     ↓
TensorRT Engine (.trt) ← This runs FAST on Jetson!
     ↓
...
Memory: 50 MB (2× less)
Speed: 2× faster (Tensor cores accelerated!)
...
Memory: 25 MB (4× less)
Speed: 4× faster (INT8 Tensor cores!)
...
- Algorithm 1: Direct convolution → 5.2 ms
- Algorithm 2: Im2Col + GEMM → 3.8 ms
- Algorithm 3: Winograd transform → 2.1 ms ← TensorRT picks this!
```
...
```bash
# FP16 precision (2× speedup, negligible accuracy loss)
...
class TRTInference:
    ...
    def infer(self, input_image):
        ...
        # Preprocessing: Resize to 640×640, normalize, etc.
...
INT8 mAP: 36.5% (0.8% loss) ← Acceptable!
```
...
5. Apply scaling to convert FP32 → INT8
```
...
# Typically: INT8 mAP@0.5 ≈ 0.365 (vs 0.373 FP32)

# Acceptable loss: < 2% relative (0.373 → 0.365 = 2.1% loss ✅)
```
...
Limitation: Can only process 640×640 images!
If you resize to 1280×1280 → ERROR ❌
```
...
# Now you can use any size from 320×320 to 1280×1280!
```
...
**Key Insights:**
- FP16: 3.7× faster than PyTorch, no accuracy loss ✅
- INT8: 6.6× faster, only 0.8% mAP loss ✅ (Best for real-time robotics!)
...
Result: Maximum performance on target ✅
```
...
**Decision:**
- If mAP > 40% is required → Use YOLOv8s
- If speed is critical (>100 FPS) → Use YOLOv8n + INT8
...
**Symptom:** TensorRT INT8 is only 2× faster than PyTorch (expected 4×).
...
1. ✅ **TensorRT optimizes models** through layer fusion, precision calibration, and kernel tuning
2. ✅ **FP16 provides 2× speedup** with negligible accuracy loss
3. ✅ **INT8 provides 4× speedup** with &lt;2% accuracy loss (requires calibration)
4. ✅ **Dynamic shapes** enable flexible input sizes at inference
5. ✅ **Always benchmark on target hardware** (Jetson Orin, not desktop GPU)
...
- Each camera outputs 640×640 images at 30 Hz
- Current: Process sequentially (28 ms per image × 4 = 112 ms total)
...
You now understand:
- ✅ NVIDIA Jetson hardware (CPU, GPU, unified memory)
- ✅ CUDA optimization techniques (coalescing, occupancy, fusion)
- ✅ TensorRT deployment (FP16/INT8 quantization, dynamic shapes)
...
Continue to **[Chapter 2.4: Power Management](../power-management/2.4.1-battery-systems.md)** 🔋⚡
